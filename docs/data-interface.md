...
content: {"status":"complete","parserVersion":1,"contentHtmlPath":"wechat/wx-.../wechat-....content.html","pageHtmlPath":"wechat/wx-.../0a1b....page.html","lastAttemptAt":1750925200000,"lastSuccessAt":1750925200000}
read: false
archived: false
...
| `content` | 可选正文状态、解析器版本、HTML sidecar 相对路径、尝试/成功时间和失败原因 |
...
微信公众号经典图文会在 Markdown 同目录保存同名 `.content.html`。它保留外层 `#js_content` 与内联样式，采集时只为展示提升 `data-src` 等懒加载属性并补全相对 URL。Markdown 仍是跨信源、方便检索的稳定正文投影；`.content.html` 是原始排版读取入口。两者均为 UTF-8 明文，不压缩、不 Base64。

`content.contentHtmlPath` 相对 `articles/`，`content.pageHtmlPath` 相对 `raw/`。读取前应把路径解析回各自根目录并拒绝越界。`content.status = failed/partial`、HTML sidecar 缺失、`parserVersion` 落后，或微信条目的本机 `pageHtmlPath`/对应页面文件缺失时，App 会在该条目下次出现在手动刷新结果中重试正文补全/页面留档。

## Raw：溯源接口

`raw/` 保存两类原始产物：

- `<encodedExternalId>.json`：采集列表阶段的 `RawItem`，包含 `sourceId/sourceType/fetchedAt/externalId/raw`。
- `<sha256>.page.html`：微信详情请求返回的完整、未改写 HTML 页面；文件名由文章外部 ID/URL 做 SHA-256 派生，避免 URL 字符进入路径。

Raw 适合排查字段映射、页面结构变化或后续离线重放，不适合直接替代 Article，因为不同信源的原始结构不统一。完整页面 HTML 只在本机保留，不进入团队同步。
...
3. 微信原始排版优先按 frontmatter 的 `content.contentHtmlPath` 读取 `.content.html`；完整页面诊断/重放按 `content.pageHtmlPath` 读取 `.page.html`。
4. `team/outbox` 与 `team/quarantine` 可能包含 Article Markdown 和直接同步的微信正文 HTML，但它们是内部可靠队列/隔离区；`team/acked` 是幂等恢复标记。消费者不要扫描、移动或删除。
5. 完整内容与元数据以 Article Markdown、其声明的内容 sidecar 和 Raw 文件为准，SQLite 只用于加速。
6. 不依赖目录遍历顺序；排序使用 `publishedAt`。
7. 不修改稳定核心字段。外部产物应写入自己的目录或系统，不要污染 infohub 真相源。
8. 若确需添加兼容注释，必须保持 frontmatter 每行 `key: <JSON value>` 格式。
