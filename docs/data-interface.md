...
3. 完整内容与元数据以 Article 文件为准，SQLite 只用于加速。
4. 不依赖目录遍历顺序；排序使用 `publishedAt`。
5. 不修改稳定核心字段。外部产物应写入自己的目录或系统，不要污染 infohub 真相源。
6. 若确需添加兼容注释，必须保持 frontmatter 每行 `key: <JSON value>` 格式。
