...
## ADR-006 Electron 升到 43（内置 Node 24）以支持 node:sqlite
- **日期**：2026-07-06
- **决策**：Electron 33 → 43。
- **理由**：真机联调发现 Electron 33 内置 Node 20，**不支持 `node:sqlite`**（Node 22.5+ 才有），启动即崩。全局规范要求用内置 sqlite（禁三方库），故升级 Electron 到内置 Node ≥22 的版本。实测 Electron 43 = Node 24.17，`node:sqlite` 可用。
- **教训**：core 测试用系统 Node 24 通过，掩盖了 Electron 运行时 Node 版本差异——联调才暴露。

## ADR-007 采集安全约束：默认手动 + 全局串行 + 极保守频率
- **日期**：2026-07-06
- **决策**：关闭自动轮询（默认纯手动刷新才采集）；`Collector` 加全局互斥锁（任何时刻只有一个 wechat 请求链）；联调期频率压到 20/时、10s 间隔、单次 1 页。
- **理由**：用户用真实公众号账号，明确要求"别拿账号做并发、别影响账号"。自动轮询定时器 + 潜在并发是最大风险，直接消除。
- **可回滚**：稳定后按需放宽频率、可选开启定时轮询（默认仍关）。
