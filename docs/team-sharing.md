...
当前桌面 v0.2.0 只接入文章 push/pull，还没有展示 assignment 或执行 lease 的“刷新我的任务”入口。因此 A/B 分配现在可通过服务端 API 管理，但要真正让桌面只采自己的任务，还需要下一阶段客户端接入；不能仅凭已有 API 宣称重复请求已经完全避免。
...
- 桌面端：加入/退出、设备凭据、可恢复历史补传、outbox/ack/隔离、定时与手动 push/pull、“我的 / 团队”视图已实现。
- 服务端：单团队入组、设备/Source/Article/contribution、幂等事件、cursor、assignment/rebalance/lease API 已实现。
- 部署：服务默认值已经固化；2026-07-13 直接访问 `/healthz` 得到空响应，说明默认入口尚未就绪。实际服务、FRP 和可信 TLS 证书不由客户端自动完成。
