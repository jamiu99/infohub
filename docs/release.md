...
- 当前源码版本为 `v0.6.0`，发布说明见 [releases/v0.6.0.md](releases/v0.6.0.md)。
- 上个正式 GitHub Release 是 `v0.5.0`；`v0.1.2` 因 preload 回归已取消并转为 draft。
...
- `v0.6.0` 继续使用现有项目 logo；新增白色/暗色主题、跨文章阅读偏好、SQLite 轻量列表和可配置团队同步周期。Article 文件与团队 `/api/v2` 不变；本地 SQLite 投影首次启动会自动重建一次。
...
checkout
  ...
  → pnpm smoke:desktop（Windows 隐藏 Electron，真实 preload/IPC）
  → electron-builder --win --publish always
  → GitHub Release（releaseType: release）
```

普通 `main` push 和 pull request 会同时运行 Ubuntu 完整门禁/Xvfb 烟测，以及 Windows 的类型检查、核心测试、构建、bundle 检查和真实 Electron 烟测；tag 只在这两套日常门禁通过后创建。

## 发布下一版本

不要复用旧 tag。例如发布 `0.6.0`：

```bash
# 1. 更新版本、文档和 release notes
pnpm version 0.6.0 --no-git-tag-version
./verify.sh

# 2. 提交并推送 main，等待普通 CI 通过
...
- 无论普通退出还是立即更新，都会先停止后续批次、等待当前网络请求及 Raw/Article/outbox 写入完成，再关闭 SQLite 和退出。
...
- `resources/branding/infohub-logo-concept-v1.png` 保留为品牌概念源图；Windows 应用 EXE、快捷方式、安装器与卸载器统一使用紧凑透明的 `resources/branding/infohub-icon-v1.png`，electron-builder 在 Windows 打包时转换为 ICO。
...
- [ ] 版本号、tag、文档一致。
- [ ] 如果本版修改团队协议，配套服务端已经先升级且部署顺序已写清；协议未变时在 Release notes 明确无需升级服务端。
