...
最后更新：**2026-07-15** · 阶段：**v0.4.1 数据生命周期与自动采集版**
...
| v0.4.1 发布门禁 | `pnpm typecheck` ✅ · `pnpm test:core` ✅（152/152）· `pnpm build`/bundle contract ✅ |
...
当前定位：**v0.4.1 已把采集证据、产品投影和外部处理结果分层，并交付图片消息、历史重跑、自动采集和可迁移资料库。旧 SSR、服务运维、多设备同步和真实 Windows 产品验收尚未闭环。**
...
- ⬜ 在 Windows 人工安装 `v0.4.1`，验收项目图标、二维码、配额、RSS、贴图号、历史重跑、资料库迁移、自动采集、三栏布局和团队范围。
- ⬜ 从已安装的 `v0.3.1` 按确认式流程更新到 `v0.4.1`，确认旧设置/凭据复制到固定 state、内容数据和 HTML sidecar 保留。
...
1. 在真实 Windows 从 `v0.3.1` 更新到 `v0.4.1`，验收旧私有状态导入、跨盘迁移、贴图号、历史重跑和自动采集多个周期。
