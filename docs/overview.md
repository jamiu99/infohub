...
- ⬜ 新建独立、自托管的 `infohub-team-server`；单实例只服务一个团队，默认端口 `18038`，公开地址必须使用 HTTPS。
- ⬜ 默认服务地址为 `https://home.agent-wiki.cn:18038`；客户端自动上传允许共享的本地采集结果，并支持“我的 / 团队”筛选。
- ⬜ 服务端用单个共享 `TEAM_TOKEN` 完成首次入组，再由服务端签发设备 token；不做邀请码系统或客户端自生成 token。
- ⬜ 团队来源去重、成员分配和短租约防止同一来源被多台设备同时采集；登录凭据永不上传。
