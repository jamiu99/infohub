...
最后更新：**2026-07-13** · 阶段：**v0.2.0 团队共享 MVP**
...
| 当前源码版本 | `v0.2.0` |
| 上个正式版本 | `v0.1.5`（提交 `a441e13`，2026-07-13）；`v0.1.2` 已撤回 |
| 自动检查 | `pnpm typecheck` ✅ · `pnpm test:core` ✅（58/58）· `pnpm build` ✅ |
...
| 团队后端 | 独立 `infohub-team-server` MVP ✅；默认公网 HTTPS 入口待部署 |
| 默认团队入口 | `https://home.agent-wiki.cn:18038/healthz` 当前返回空响应，尚不可用于客户端入组 |

当前定位：**个人采集链路与团队共享 MVP 均可运行、可构建；默认服务部署、多设备同步和真实 Windows 产品验收尚未闭环。**
...
- ⬜ 在 Windows 人工安装 `v0.2.0`，验收二维码、配额、RSS、阅读、归档、团队加入和“我的 / 团队”。
- ⬜ 从已安装的 `v0.1.5` 按确认式流程更新到 `v0.2.0`，确认重启安装、设置和账号数据保留。
...
- ⬜ 部署默认 `https://home.agent-wiki.cn:18038`，完成证书、FRP、数据目录备份与健康检查。
...
1. 部署并验证默认团队服务的可信 HTTPS 入口。
2. 用两台 Windows 设备完成 v0.1.5 → v0.2.0 更新与团队同步验收。
