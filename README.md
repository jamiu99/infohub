...
- 微信经典图文页用 HTML 树提取 `#js_content`；图片消息（“贴图号”）读取正文图片数组；详情默认显示原始排版，也可切换到 Markdown 阅读版。
...
当前源码版本是 `v0.4.0`。本版补齐图片消息、历史正文重跑、用户显式开启的自动采集和可迁移资料库，并继续使用项目 logo 与传统“检查—确认—下载—重启”更新流程。`v0.3.0` 起团队正文 HTML 同步硬切 `/api/v2`，不提供 v1 fallback，也不支持旧桌面端或旧服务端混用；自托管团队必须先升级服务端，再升级全部桌面端。复杂动态微信组件、Windows 跨盘迁移、跨版本自动更新和多设备同步仍需人工验收，详见 [docs/overview.md](docs/overview.md) 与 [v0.4.0 发布说明](docs/releases/v0.4.0.md)。
