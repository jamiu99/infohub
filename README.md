...
当前源码版本是 `v0.5.0`。本版升级米色扁平化阅读体验，新增按当前范围一键已读和独立“来源与抓取”设置页，并把“重抓本篇”“拉取最新”“处理已入库历史”的范围与联网行为明确分开。`v0.3.0` 起团队正文 HTML 同步硬切 `/api/v2`，不提供 v1 fallback，也不支持旧桌面端或旧服务端混用；自托管团队必须先升级服务端，再升级全部桌面端。微信公众号未入库旧文章回溯、复杂动态组件、跨版本自动更新和多设备同步仍需人工验收，详见 [docs/overview.md](docs/overview.md) 与 [v0.5.0 发布说明](docs/releases/v0.5.0.md)。
